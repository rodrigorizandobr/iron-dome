...
export class I18nService {
  ...
  private loadTranslations() {
    try {
      ...
      if (fs.existsSync(filePath)) {
        this.translations = JSON.parse(fs.readFileSync(filePath, 'utf8')) as Record<
          string,
          unknown
        >;
      } else {
        ...
        this.translations = JSON.parse(fs.readFileSync(fallbackPath, 'utf8')) as Record<
          string,
          unknown
        >;
      }
  ...
  translate(key: string, args: Record<string, string> = {}): string {
    const value: string | undefined = key.split('.').reduce(
