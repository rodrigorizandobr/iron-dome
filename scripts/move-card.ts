...
try {
  ...
  const projJson = execSync(`gh project view ${projectNumber} --owner ${owner} --format json`, { encoding: 'utf8' });
  const projectId = JSON.parse(projJson).id;

  // 2. Descobre os IDs do Campo "Status" e da Opção (Coluna) desejada
  const fieldsJson = execSync(`gh project field-list ${projectNumber} --owner ${owner} --format json`, { encoding: 'utf8' });
  const fields = JSON.parse(fieldsJson).fields;
  
  const statusField = fields.find((f: any) => f.name === 'Status');
  const option = statusField.options.find((o: any) => o.name.toLowerCase() === targetColumn.toLowerCase());
  ...
  const issueJson = execSync(`gh issue view ${issueNumber} --json projectItems`, { encoding: 'utf8' });
  ...
  if (!itemId) {
     ...
     const addJson = execSync(`gh project item-add ${projectNumber} --owner ${owner} --url https://github.com/${repoInfo}/issues/${issueNumber} --format json`, { encoding: 'utf8' });
     itemId = JSON.parse(addJson).id;
  }

  // 4. Executa a movimentação com os IDs exatos
  execSync(`gh project item-edit --id ${itemId} --project-id ${projectId} --field-id ${statusField.id} --single-select-option-id ${option.id}`);
